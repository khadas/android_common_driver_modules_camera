...
static struct cam_device * video_to_cam_device(struct aml_video *video)
{
...
{
	struct aml_video *video = video_drvdata(file);
	struct cam_device * cam_dev = video_to_cam_device(video);
	int ret = 0x0;

	if (0 == video->first_frame_logged) {
		dev_err(video->dev, "video %d beg dq first 1 frame\n", video->id);
	}
	//mod_timer(&cam_dev->dq_check_timer, jiffies + 1*HZ); // 1/2 s = 500ms timeout
...
static int video_start_streaming(struct vb2_queue *queue, unsigned int count)
{
	...
	while (1) {
		...
		if (rtn < 0 && rtn != -ENOIOCTLCMD) {
			...
			dev_err(video->dev, "Failed to call stream on , ret %d\n", rtn);
			goto error_return;
		}
	}
