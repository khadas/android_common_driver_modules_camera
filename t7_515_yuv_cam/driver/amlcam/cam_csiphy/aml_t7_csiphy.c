...
static int csiphy_of_parse_dev(struct csiphy_dev_t *csiphy_dev)
{
	...
	csiphy_dev->csi_host = csiphy_ioremap_resource(csiphy_dev, "csi_phy");
	if (!csiphy_dev->csi_host) {
		rtn = -EINVAL;
		goto error_rtn;
	}

	csiphy_dev->csi_dphy = csiphy_dev->csi_host + CSIPHY_DPHY_TO_HOST_OFFSET;

	csiphy_dev->mipi_clk0 = devm_clk_get(csiphy_dev->dev, "mipi_phy_clk");
...
static int csiphy_subdev_stream_on(struct csiphy_dev_t *csiphy_dev)
{
	...
	csiphy_dev->mipi_data_lanes = nlanes;
	csiphy_dev->mipi_link_freq = link_freq;
	return csiphy_dev->ops->hw_start(csiphy_dev, csiphy_dev->mipi_data_lanes, csiphy_dev->mipi_link_freq);
}
...
#endif
{
	...
	csiphy_dev->pfmt[fmt->pad] = fmt->format;
