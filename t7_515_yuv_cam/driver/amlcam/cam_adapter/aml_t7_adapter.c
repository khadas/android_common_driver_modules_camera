...
static irqreturn_t adap_interrupt_handler(int irq, void *dev)
{
	...
	//adap_isr_printk("fe0 , status 0x%x\n", status);
	...
	do {
		if (status & (0x1 << 18)) {
			video = &adap_dev->video[AML_ADAP_STREAM_FIRST_VC0];
		...
		if (status & (0x1 << 21)) {
			video = &adap_dev->video[AML_ADAP_STREAM_FIRST_VC1];
			if (video->ops->cap_irq_handler) {
				video->ops->cap_irq_handler(video, status);
			} else {
				adap_isr_printk("error no no video cap irq handler \n");
				...
				struct v4l2_mbus_framefmt *format)
{
	...
	adap_fmt = adap_subdev_convert_fmt(adap_dev, format);
	if (adap_fmt < 0) {
