						 ...
						 struct aml_crop *input, struct aml_format *output)
{
...
#ifdef T7C_CHIP
	pre_vscale_max_hsize = reg_prevsc_flt_num == 2 ? max_hsize : reg_prevsc_flt_num == 4 ? max_hsize / 2
																						 : max_hsize / 4;
	...
	reg_prevsc_flt_num = (prev_flt_4_invld | prev_flt_8_invld) & prev_en ? (reg_prevsc_flt_num >> 1) : reg_prevsc_flt_num; // reg_prevsc_flt_num = 2/4/8
#else
	...
	reg_prevsc_flt_num = ((reg_prevsc_flt_num == 4) & prev_flt_4_invld) | ((reg_prevsc_flt_num == 8) & prev_flt_8_invld) ? (reg_prevsc_flt_num >> 1) : reg_prevsc_flt_num; // reg_prevsc_flt_num = 2/4/8

	pre_hsc_mult = 1 << reg_prehsc_rate;
	ihsize_after_pre_hsc = preh_en | preh_force_open ? (ihsize + pre_hsc_mult - 1) / pre_hsc_mult : ihsize;
#endif
	pre_vscale_max_hsize = reg_prevsc_flt_num == 2 ? max_hsize : reg_prevsc_flt_num == 4 ? max_hsize / 2
																						 : max_hsize / 4; // LBUF_MAX_HSIZE/2
	...
	reg_vsc_tap_num_alt = prev_en ? (ohsize_range == 2	 ? 2
									 : ohsize_range == 1 ? (reg_vsc_tap_num > 4 ? 4 : reg_vsc_tap_num)
														 : reg_vsc_tap_num)
								  : (ohsize_range == 3	 ? 2
									 : ohsize_range == 2 ? (reg_vsc_tap_num > 4 ? 4 : reg_vsc_tap_num)
														 : reg_vsc_tap_num);
...
void isp_disp_calc_slice(struct isp_dev_t *isp_dev, u32 idx, struct aml_slice *param)
{
	...
	hscale_rate = (reg_prehsc_rate == 0) ? 1 : (reg_prehsc_rate == 1) ? 2
										   : (reg_prehsc_rate == 2)	  ? 4
																	  : 8;
	...
	if (reg_crop_en == 1) {
		...
		right_hsize = crop_hend < whole_frame_hcenter ? 0 : (reg_crop_hstart > whole_frame_hcenter) ? (crop_hend - reg_crop_hstart)
																									: crop_hend;
	...
	if (reg_hsc_en) {
		...
		post_h_posi[1] = (reg_hsc_yuv444to422_en == 1) ? (tmp_left_hsize / 2 - tmp_left_overlap / 2 - tmp_right_overlap / 2) : post_h_posi[0];
